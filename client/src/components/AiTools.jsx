  ...
  return (
    <div className='px-4 sm:px-20 xl:px-32 my-24'>
        <div className='text-center'>
            ...
            <p className='text-gray-500 max-w-lg mx-auto'>Everything you need to create, enhace, and optimize your content with cutting-edge AI technology. </p>
        </div>
        ...
        <div className='flex flex-wrap mt-10 justify-center'>
            {AiToolsData.map((tool, index)=>(
                <div key={index} className='p-8 m-4 max-w-xs rounded-lg bg-[#FDFDFE] shadow-lg border border-gray-100 hover:-translate-y-1 transition-all duration-300 cursor-pointer' onClick={()=> user && navigate(tool.path)} >
                    ...
                    <h3 className='mt-6 mb-3 text-gray-800 text-lg font-semibold'>{tool.title}</h3>
                    <h3 className='text-gray-500 text-sm max-w-[95%]'>{tool.description}</h3>
                </div>
            ))}
        </div>
    </div>
